...
class TuPlot(QDockWidget, Ui_tuflowqgis_TuPlot):
	...
	def __init__(self, iface):
		...
		self.cbDeactivate.stateChanged[int].connect(self.deactivate_changed)
	...
	def add_res(self):
		...
		for x in range(len(inFileNames[0])):
			fpath, fname = os.path.split(inFileNames[0][x])
			...
			#self.lwStatus.item(0).setTextColor(self.qgreen)
			root, ext = os.path.splitext(inFileNames[0][x])
			if ext.upper()=='.INFO':
				self.lwStatus.insertItem(0,'.INFO file detected - using TUFLOW_Results2013.')
				#self.lwStatus.item(0).setTextColor(self.qgreen)
				#self.lwStatus.insertItem(0,'Loading...')
				try:
					#QMessageBox.information(self.iface.mainWindow(),"Debug", "line406")
					res = TUFLOW_results2013.ResData()
					res.Load(inFileNames[0][x], self.iface)
					self.res.append(res)
	...
	def add_res_gis(self):
		...
		if len(inFileNames[0]) < 1: #empty list
			return
		self.lwStatus.insertItem(0,'Number of files: '+str(len(inFileNames)))
		for x in range(len(inFileNames[0])):
			# Store the path we just looked in
			inFileName = inFileNames[0][x]
		...
		if res.GIS.P:
			if res.Index.nPoints>0: #2017-08-AA added check for data in file
				...
				try:
					addLayer = self.iface.addVectorLayer(fullfile, os.path.basename(fullfile)[:-4], "ogr")
		...
		if res.GIS.L:
			if res.Index.nLines>0: #2017-08-AA added check for data in file
				addLayer = fullfile = os.path.join(fpath,res.GIS.L)
				try:
					addLayer = self.iface.addVectorLayer(fullfile, os.path.basename(fullfile)[:-4], "ogr")
					# apply tuflow style to imported gis layer
					tf_styles = tuflowqgis_styles.TF_Styles()
		...
		if res.GIS.R:
			if res.Index.nRegions>0: #2017-08-AA added check for data in file
				...
				try:
					self.iface.addVectorLayer(fullfile, os.path.basename(fullfile)[:-4], "ogr")
				except:
					self.lwStatus.insertItem(0,'Failed to load GIS layer')
					self.lwStatus.item(0).setTextColor(self.qblue)
			else:
	...
	def update_reslist(self):
		...
		for x in range(0, self.ResList.count()):
			self.ResList.item(x).setSelected(True)
	...
	def layerChanged(self):
		...
		if self.cbForceXS.isChecked():
			...
			self.cLayer = self.canvas.currentLayer()
			#QMessageBox.information(self.iface.mainWindow(), "Opened", str(type(self.cLayer)))
		...
		if xs_format: # need to get sections
			...
			if self.cLayer and (self.cLayer.type() == QgsMapLayer.VectorLayer):
				...
				#index to ID
				self.idx = self.cLayer.fields().lookupField('ID')
				if (self.idx < 0):
					self.locationDrop.clear()
					self.locationDrop.addItem("Not a TUFLOW Layer Selected")
		...
		if self.handler:
			...
			self.selected_layer.selectionChanged.disconnect(self.select_changed)
			self.handler = False
			self.selected_layer = None
			...
			if self.cLayer:
				...
				try:
					for feature in self.cLayer.selectedFeatures():
						...
						error, message = self.XS.addfromfeature(fpath, fields, feature, self.iface)
						if error:
	...
	def loc_changed(self):
		...
		#QMessageBox.information(self.iface.mainWindow(), "Debug", str(loc))
		self.ResTypeList.clear()
		...
		try:
			...
			self.listTime.item(0).setSelected(True)
			#self.listTime.setItemSelected(item, True)
		except:
			self.lwStatus.insertItem(0,'WARNING - Unable to populate times, check results loaded.')
			#self.lwStatus.item(0).setTextColor(self.qblue)
		#QMessageBox.information(self.iface.mainWindow(), "Debug", str(type(self.ResTypeList.item(0))))
		if self.ResTypeList.item(0) is not None:
		...
		if start:
			try:
				...
				for x in range(0, self.listTime.count()):
					self.listTime.item(x).setSelected(True)
					#self.listTime.setItemSelected(item, True)
					self.draw_figure()
					filenum = str(x+1)
					...
					self.listTime.item(x).setSelected(False)
	...
	def showIt(self):
		...
		#	mpltoolbar = matplotlib.backends.backend_qt5agg.NavigationToolbar2QTAgg(self.plotWdg, self.frame_for_toolbar)
		#else:
		#	mpltoolbar = matplotlib.backends.backend_qt5agg.NavigationToolbar2QT(self.plotWdg, self.frame_for_toolbar)
		mpltoolbar = matplotlib.backends.backend_qt5agg.NavigationToolbar2QT(self.plotWdg, self.frame_for_toolbar)
						...
						if typenames.count('Bed Level')!=0 and nres_used==len(reslist):
							...
							if res.formatVersion >= 2: #2015
								for verts in res.LP.culv_verts:
									if verts:
										#QMessageBox.information(self.iface.mainWindow(), "DEBUG", str(len(verts)))
