...
class TuResults2D():
	...
	def loadMeshLayer(self, fpath, **kwargs):
		...
		if fext.lower() == '.xmdf' or fext.lower() == '.dat' or fext.lower() == '.2dm':
			...
			QMessageBox.information(self.iface.mainWindow(), "TUFLOW Viewer", "Must select a .xmdf .dat or .2dm file type")
			return None, None, False
			...
			if '/maximums' in mdGroup.name().lower() or \
				...
				for j in range(dp.datasetCount(i)):
					...
					if md.time() == 900001.0:  # time of peak h
						if mdGroup.name() in results[name]:
							del results[name][mdGroup.name()]
						...
						results[name][mdGroup.name()]['{0:.6f}'.format(md.time())] = \
						...
						if ext.upper() == '.DAT' and mdGroup.isVector():  # need to add result type again as vector type
							md = dp.datasetMetadata(QgsMeshDatasetIndex(i, j))  # metadata for individual timestep
							results[name]['{0} Vector'.format(mdGroup.name())]['{0:.6f}'.format(md.time())] = \
