...
class XS():
	...
	def addfromfeature(self,fpath,fields,feature, qgis):
		error = False
		message = None
		# get field info
		if len(fields) < 9:
		...
		try:
			...
			if not col6: # is QGIS variant null
				col6 = None
			#QMessageBox.information(qgis.mainWindow(),"Debug", "completed info from fields")
		except:
			error = True
		...
		try:
			#QMessageBox.information(qgis.mainWindow(),"Debug", "line299")
			self.nXS = self.nXS + 1
