...
class TestSWMMShapeConvert(unittest.TestCase):
    ...
    def test_inconsistent_case(self):
        # If we encounter inconsistent case, we will use the case of the primary input and change referenced tables
        # For example, we will force conduits to use the node case used in the junction or outfall table
        base_filename = 'Swmm_site_drainage_model_inconsistent_case'
        input_file = get_input_full_filenames(
            [
                f'{base_filename}.inp',
            ]
